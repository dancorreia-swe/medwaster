  ...
  try {
    ...
    const sampleQuestions = await db.insert(questions).values([
      ...
      // True/False Questions
      {
        ...
        status: "active",
        categoryId: categoryIds[0], // Geografia
        explanation: "Verdadeiro. O Brasil ocupa cerca de 47% do território sul-americano.",
        createdBy: "admin",
        updatedBy: "admin"
      },
      {
        prompt: "A fotossíntese é o processo pelo qual as plantas produzem seu próprio alimento.",
        type: "true_false",
        difficulty: "intermediate", 
        status: "active",
        categoryId: categoryIds[2], // Ciências
        explanation: "Verdadeiro. As plantas convertem luz solar, água e CO2 em glicose.",
        createdBy: "admin",
        updatedBy: "admin"
      },
      ...
      // Fill in the blank
      {
        ...
        status: "active",
        categoryId: categoryIds[2], // Ciências
        explanation: "H2O representa dois átomos de hidrogênio e um de oxigênio.",
        createdBy: "admin",
        updatedBy: "admin"
      },
    ...
    await db.insert(questionOptions).values([
      { questionId: sampleQuestions[0].id, label: "A", content: "São Paulo", isCorrect: false },
      { questionId: sampleQuestions[0].id, label: "B", content: "Rio de Janeiro", isCorrect: false },
      { questionId: sampleQuestions[0].id, label: "C", content: "Brasília", isCorrect: true },
      { questionId: sampleQuestions[0].id, label: "D", content: "Salvador", isCorrect: false }
    ]);

    // Maior planeta options
    await db.insert(questionOptions).values([
      { questionId: sampleQuestions[1].id, label: "A", content: "Terra", isCorrect: false },
      { questionId: sampleQuestions[1].id, label: "B", content: "Júpiter", isCorrect: true },
      { questionId: sampleQuestions[1].id, label: "C", content: "Saturno", isCorrect: false },
      { questionId: sampleQuestions[1].id, label: "D", content: "Netuno", isCorrect: false }
    ]);

    // Independência do Brasil options
    await db.insert(questionOptions).values([
      { questionId: sampleQuestions[2].id, label: "A", content: "1822", isCorrect: true },
      { questionId: sampleQuestions[2].id, label: "B", content: "1825", isCorrect: false },
      { questionId: sampleQuestions[2].id, label: "C", content: "1889", isCorrect: false },
      { questionId: sampleQuestions[2].id, label: "D", content: "1500", isCorrect: false }
    ]);

    // Math question options
    await db.insert(questionOptions).values([
      { questionId: sampleQuestions[6].id, label: "A", content: "110", isCorrect: false },
      { questionId: sampleQuestions[6].id, label: "B", content: "120", isCorrect: true },
      { questionId: sampleQuestions[6].id, label: "C", content: "130", isCorrect: false },
      { questionId: sampleQuestions[6].id, label: "D", content: "140", isCorrect: false }
    ]);
