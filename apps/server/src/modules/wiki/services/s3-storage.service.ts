...
// Configuration
const BUCKET_NAME = S3_BUCKETS.WIKI;
const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB for wiki files (larger than questions)
...
export class WikiS3StorageService {
  /**
  ...
  /**
   * Determines the folder based on MIME type
   */
  private static getFolder(mimeType: string): string {
   ...
   */
  static async uploadFile(file: File): Promise<{ url: string; filename: string; key: string }> {
    this.validateFile(file);
    await this.ensureBucket();

    const filename = this.generateFilename(file.name);
    const folder = this.getFolder(file.type);
