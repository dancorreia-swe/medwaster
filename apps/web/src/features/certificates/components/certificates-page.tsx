...
export function CertificatesPage() {
	...
	const [selectedCertificate, setSelectedCertificate] = useState<CertificateTableItem | null>(null);
	...
	const approveMutation = useMutation({
		...
		onSuccess: () => {
			...
			queryClient.invalidateQueries({ queryKey: certificatesQueryKeys.pending() });
			queryClient.invalidateQueries({ queryKey: certificatesQueryKeys.stats() });
			...
			<header className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
				<div className="space-y-1">
					...
					<p className="text-sm text-muted-foreground max-w-2xl">
						Gerencie e aprove os certificados gerados automaticamente quando os alunos completam todas as trilhas.
					</p>
				</div>
			</header>
