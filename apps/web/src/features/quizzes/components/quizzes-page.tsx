...
import { quizzesListQueryOptions, useArchiveQuiz, useDeleteQuiz } from "../api";
import { QuizzesTable } from "./quizzes-table";
import {
  QuizFiltersBar,
  type QuizFilters,
} from "./quiz-filters-bar";
...
export function QuizzesPage() {
  ...
  const { data, isLoading, isFetching, error } = useQuery(
    quizzesListQueryOptions(filters),
  );
  ...
  const handleFiltersChange = (newFilters: QuizFilters) => {
    setFilters((prev) => {
      ...
      const updated: QuizListQueryParams & QuizFilters = {
        page: 1,
      ...
      Object.entries(prev).forEach(([key, value]) => {
        if (!['page', 'pageSize'].includes(key) && !(key in newFilters) && value !== undefined) {
  ...
  const handleArchive = async (quiz: QuizListItem) => {
    try {
      await archiveMutation.mutateAsync(quiz.id);
    } catch (error) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Gerencie quizzes e avaliações do sistema
          </p>
        </div>
        <Button asChild>
      ...
      <div className="relative">
        ...
        {error ? (
          <Alert variant="destructive">
            <AlertDescription>
              Erro ao carregar quizzes. Tente novamente.
            </AlertDescription>
          </Alert>
        ) : isLoading ? (
          <QuizzesSkeleton />
